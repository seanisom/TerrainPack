...
#define __TBB_EXCEPTION_PTR_PRESENT  (_MSC_VER >= 1600 || __GXX_EXPERIMENTAL_CXX0X__ && (__GNUC__==4 && __GNUC_MINOR__>=4))
        ...
        #error Current runtime does not support std::exception_ptr. Set TBB_USE_CAPTURED_EXCEPTION and make sure that your code is ready to catch tbb_graphics_samples::captured_exception.
    #endif
...
#define __TBB_DEFAULT_PARTITIONER tbb_graphics_samples::simple_partitioner
#else
/** Default partitioner for parallel loop templates in TBB 2.2 */
#define __TBB_DEFAULT_PARTITIONER tbb_graphics_samples::auto_partitioner
...
#if INTEL_PRIVATE
//  Intel(R) Premier Support Issue 577434
#endif
