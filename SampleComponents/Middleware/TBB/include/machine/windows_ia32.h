...
#if INTEL_PRIVATE && 0
// Keeping an experimental inline implementation for 64-bit atomic operations.
// If we ever try using it, it will have to be carefully reviewed.
...
static inline __int64 __TBB_machine_load8 ( const volatile void *ptr ) {
    __int64 result;
    volatile __int64 *p = (__int64 *)ptr;
    __asm 
   {
       fildq [p]
       fistpq result
...
static inline void __TBB_machine_store8 ( const volatile void *ptr, __int64 value ) {
    __int64 result;
    volatile __int64 *p = (__int64 *)ptr;
    __asm 
    {
