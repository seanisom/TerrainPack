    ...
    #define __TBB_DEFINE_PROFILING_SET_NAME(sync_object_type)    \
        namespace profiling {                                                       \
            inline void set_name( sync_object_type& obj, const wchar_t* name ) {    \
                tbb_graphics_samples::internal::itt_set_sync_name_v3( &obj, name );                  \
            }                                                                       \
            inline void set_name( sync_object_type& obj, const char* name ) {       \
                size_t len = tbb_graphics_samples::internal::multibyte_to_widechar(NULL, name, 0);   \
                wchar_t *wname = new wchar_t[len];                                  \
                tbb_graphics_samples::internal::multibyte_to_widechar(wname, name, len);             \
                set_name( obj, wname );                                             \
                delete[] wname;                                                     \
            }                                                                       \
        }
...
namespace tbb_graphics_samples {
    namespace internal {
        ...
        // two template arguments are to workaround /Wp64 warning with tbb_graphics_samples::atomic specialized for unsigned type
        template <typename T, typename U>
        inline void itt_store_word_with_release(tbb_graphics_samples::atomic<T>& dst, U src) {
        ...
        template <typename T>
        inline T itt_load_word_with_acquire(const tbb_graphics_samples::atomic<T>& src) {
#if TBB_USE_THREADING_TOOLS
            // This assertion should be replaced with static_assert
            __TBB_ASSERT(sizeof(T) == sizeof(void *), "Type must be word-sized.\n");
