...
private:
    ...
    //TODO: Inherit from std::iterator<std::random_access_iterator_tag,Value>
    //      There is a defective warning in gcc 4.1.2 (SuSe 10 and RHEL 5.1 Tikanga)
#endif//INTEL_PRIVATE
    {
        ...
        friend class tbb_graphics_samples::concurrent_vector;
#else
...
T& concurrent_vector<T, A>::internal_subscript( size_type index ) const {
    ...
    T* array = static_cast<T*>( tbb_graphics_samples::internal::itt_hide_load_word(my_segment[k].array));
...
#if INTEL_PRIVATE
/*******************************************************
Changes in tbb_graphics_samples::concurrent_vector since 2.0 version:
