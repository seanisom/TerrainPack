...
namespace DXTC
{
	...
	void CompressImageDXT5(const BYTE* inBuf, BYTE* outBuf, int width, int height, unsigned int rowPitch)
	{
		ALIGN16(BYTE block[64]);
		ALIGN16(BYTE minColor[4]);
		ALIGN16(BYTE maxColor[4]);
	...
	void CompressImageDXT5SSE2(const BYTE* inBuf, BYTE* outBuf, int width, int height, unsigned int rowPitch)
	{
		ALIGN16(BYTE block[64]);
		ALIGN16(BYTE minColor[4]);
		ALIGN16(BYTE maxColor[4]);
	...
	// This function does not find the minimum and maximum alpha values.
	...
	// Assigns indices to the colors in colorBlock.
	...
	// Copies a 4x4 block of pixels from the input buffer into a 4x4 temporary buffer.
	...
	// Finds the bounding box of the color block.
